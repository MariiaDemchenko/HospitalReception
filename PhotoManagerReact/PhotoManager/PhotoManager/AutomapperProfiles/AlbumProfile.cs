...
namespace PhotoManager.AutomapperProfiles
{
    public class AlbumProfile : Profile
    {
        public AlbumProfile(string imagesPath, string thumbsPath)
        {
            ...
            CreateMap<AlbumViewModel, Album>().ForMember(album => album.Cover, opt => opt.MapFrom(c => string.Empty))
                .ForMember(album => album.Id, opt => opt.MapFrom(c => c.AlbumId.ToString()));
        }
    }
}